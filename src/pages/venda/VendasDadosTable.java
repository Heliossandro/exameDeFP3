...
public class VendasDadosTable extends AbstractHashTableCoalashed
{
   public VendasDadosTable(String fileName, int tableSize)
   {
		super(fileName, tableSize);	
   }	
   public VendasDadosTable( )
   {
   ...
   {
	VendasPNode node = (VendasPNode)getEmptyNode();
...
public VendasPNode getNode(String key) throws NullPointerException
{		
	//calcula a posicao de entrada na tabela apartir da chave key
	int tablePosition = calcularHashCode( key );

	VendasPNode tmp = (VendasPNode)getNode( tablePosition );
...
{
	VendasDadosTable table = new VendasDadosTable("vendas.dat", 100);
	
	VendasPNode tmp = table.getNode(key);
...
public static Vector getAllNodes()
{
	...
	VendasDadosTable hashProduto = new VendasDadosTable("vendas.dat", 100);
	
	VendasPNode tmp = new VendasPNode();
	...
	try
	{
		...
		for(int i=0; i < hashProduto.tableSize; i++)
		{
			...
			if( !tmp.isEmptyNode()  )
			{
				...
				System.out.println( tmp.getModel().getProduto());
			}
			else
...
public void adicionarNovoVenda(VendasPNode node)
{
	try
	{
		stream.seek(0);
...
// sobrepoem 1 registo
public void sobrePorRegisto(VendasPNode node, int posTabela)
{	
	VendasPNode no = (VendasPNode)getNode(posTabela);	
...
public void adicionarNaListaColisoes(VendasPNode node, int lastColision)
{		
	VendasPNode no = (VendasPNode)getNode(lastColision);
...
// para escrever o proximo registo
 ...
 {
	return (2 * 4 + tablePosition * VendasPNode.sizeof());
 }


public SaveWriteReadInteface getEmptyNode()
{
	return new VendasPNode();
}
...
{
	VendasDadosTable hashProduto = new VendasDadosTable("vendas.dat",100);
	VendasPNode tmp = (VendasPNode)hashProduto.getEmptyNode();
	String output = "\n";
	...
	try
	{
			...
			JOptionPane.showMessageDialog(null, new JScrollPane(area), "Listagem de Vendas", JOptionPane.INFORMATION_MESSAGE);
